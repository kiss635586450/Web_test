...
def get_number(lenght):
    """"
    获取指定长度的数字，类型是字符串
    :param length: 将要生成的字符长度
    :return: 生成好的随机字符串
    """
    return ''.join(__get_randoms(lenght, numbers))
...
def get_string(length):
    """
       ...
       :return: 生成好的随机字符串
    """
    return ''.join(__get_randoms(length, lowercase_letters+majuscule+numbers))
